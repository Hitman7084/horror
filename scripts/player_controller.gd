...
func _ready() -> void:
	...
	# Ramp traversal: 45-degree slope needs floor_max_angle above default (exactly 45 deg).
	floor_max_angle = deg_to_rad(50.0)
	# Keep horizontal speed constant on slopes (prevents slowdown going uphill).
	floor_constant_speed = true
	# Snap-down keeps the character grounded when descending the ramp.
